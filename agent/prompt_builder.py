...
PLATFORM_HINTS = {
    ...
    "inkbox": (
        ...
        "\n"
        "POST-CALL REFLECTION (Inkbox voice): when a voice call ends, the "
        "adapter enqueues a synthetic ``[call_ended]`` turn so you can do "
        "follow-up work. Hard rules:\n"
        "  • If you committed to anything during the call — \"I'll send "
        "you the YC email\", \"I'll text you the link\", \"I'll call you "
        "back tomorrow\" — perform it now via TOOL CALLS, not by promising "
        "again in text. Side effects (email, SMS, cron, memory write) "
        "happen only through tools; text replies in this turn are "
        "suppressed because the caller already hung up.\n"
        "  • Use ``cronjob create deliver=local`` for time-delayed work "
        "(schedule a callback in 1h, send a digest tomorrow morning).\n"
        "  • Use ``execute_code``/``terminal`` with the Inkbox SDK for "
        "immediate sends (send an email now with the YC one-liner, save "
        "a note on the contact).\n"
        "  • If nothing needs doing, reply with exactly ``[SILENT]`` and "
        "no other text.\n"
        "  • A spoken in-call promise without a corresponding tool call "
        "afterwards is a hallucination — the user heard you commit, then "
        "nothing happens. Always close the loop here."
    ),
    "mattermost": (
